...
def get_last_message_info(lead_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT role, timestamp FROM conversations WHERE lead_id = ? ORDER BY timestamp DESC LIMIT 1",
        (lead_id,)
    )
    row = cursor.fetchone()
    conn.close()
...
QUICK_REPLIES = [
    ...
    "📐 What is the carpet area of 2BHK / 3BHK?",
    "💰 What are the pricing options?",
    "📅 I'd like to book a site visit",
]

GREETING = (
    "👋 Hi! I'm your Aurelia Heights AI assistant. "
    "How may I help you today? You can ask me anything about the project "
    "or pick one of the quick options below."
)

NUDGE = (
    "👋 Still there? I'm here to help! "
    "Here are a few things I can tell you about Aurelia Heights:"
)
...
with tab3:
    ...
    if leads_df.empty:
        ...
        if chat_key not in st.session_state:
            history = get_chat_history(selected_lead_id)
            if not history:
                # Brand-new conversation — inject greeting (no LLM call)
                save_chat_message(selected_lead_id, "assistant", GREETING)
                history = [{"role": "assistant", "content": GREETING}]
            st.session_state[chat_key] = history
            st.session_state[nudge_key] = False  # nudge not yet sent

        # ── Inactivity nudge check (2-minute rule) ──
        last_info = get_last_message_info(selected_lead_id)
        if last_info:
            last_role, last_ts_str = last_info
            try:
                last_ts = datetime.strptime(last_ts_str, "%Y-%m-%d %H:%M:%S")
            except ValueError:
                last_ts = datetime.strptime(last_ts_str[:19], "%Y-%m-%dT%H:%M:%S")
            gap = (datetime.now() - last_ts).total_seconds()
            # Nudge if: last msg was from user, gap > 2 min, not already nudged this session
            if last_role == "user" and gap > 120 and not st.session_state.get(nudge_key, False):
                save_chat_message(selected_lead_id, "assistant", NUDGE)
                st.session_state[chat_key].append({"role": "assistant", "content": NUDGE})
                st.session_state[nudge_key] = True

        # Reset nudge flag when user sends a new message (handled below via session flag)
        for msg in st.session_state[chat_key]:
            with st.chat_message("user" if msg["role"] == "user" else "assistant"):
                st.markdown(msg["content"])

        # ── Quick reply buttons ──
        # Show after greeting (0 user msgs) OR after a nudge (last msg is NUDGE)
        user_msgs = [m for m in st.session_state[chat_key] if m["role"] == "user"]
        last_bot_msg = next(
            (m["content"] for m in reversed(st.session_state[chat_key]) if m["role"] == "assistant"), ""
        )
        show_quick = len(user_msgs) == 0 or last_bot_msg == NUDGE

        if show_quick:
            st.markdown("**Quick options — tap to start:**")
            qcols = st.columns(2)
            for qi, qopt in enumerate(QUICK_REPLIES):
                if qcols[qi % 2].button(qopt, key=f"qr_{qi}_{selected_lead_id}_{len(user_msgs)}"):
                    st.session_state[f"quick_input_{selected_lead_id}"] = qopt
                    st.rerun()

        # Consume a quick-reply click the same way as typed input
        auto_prompt = st.session_state.pop(f"quick_input_{selected_lead_id}", None)

        if prompt := (auto_prompt or st.chat_input("Message the AI Agent...")):
            # Reset nudge flag — user is now active again
            st.session_state[nudge_key] = False
            ...
            if msg_count > 0 and (msg_count == 1 or msg_count % 3 == 0):
                ...
                real_history = [
                    ...
                    if not (m['role'] == 'assistant' and m['content'] in (GREETING, NUDGE))
                ]
